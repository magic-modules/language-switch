...
var LanguageSwitch = function LanguageSwitch() {
  var props = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var _props$languages = props.languages,
  ...
  var urlArr = url.split('/').filter(function (a) {
    return a;
  });
  var urlLang = urlArr[0];
  var lang = languages.find(function (l) {
    return l.code === urlLang;
  }) || languages[0] || {
    code: props.language
  };
  ...
  return ul({
  ...
  }, languages.map(function (_ref13) {
    var _ref13$to = _ref13.to,
    ...
    url = url.replace("/".concat(language, "/"), '/');
    var h = hash ? "#".concat(hash) : '';
    to = (root + to + url + h).replace(/\/\/+/g, '/');
    return li([Link({
      to: to,
      onclick: [actions.changeLanguage, code]
    }, text)]);
  }));
};
...
var actions = {
  ...
  'gdpr': {
    ...
    'toggleAllow': function toggleAllow(state, _ref20) {
  ...
  'pre': {
    'clip': function clip(state, _ref21) {
...
var pages = {
  '/language-switch/': function languageSwitch(state) {
    return [LanguageSwitch(state), h1('@magic-modules/language-switch'), p(['this is the ', Link({
      to: 'https://github.com/magic-modules'
    }, '@magic-modules'), ' LanguageSwitch component. It provides a language switch menu.']), GitBadges('magic-modules/language-switch'), h2({
      id: 'installation'
    }, 'installation:'), Pre('npm install @magic-modules/language-switch'), h2({
      id: 'usage'
    }, 'usage:'), p('in a page or module View'), Pre("export const View = state => div({ class: 'page' }, LanguageSwitch(state))"), h2({
      id: 'pages'
    }, 'language pages'), p('create your languages in the pages directory, for example:'), Pre("\n/pages/\n  index.mjs // english page\n  /de/\n    index.mjs // german page\n"), h2({
      id: 'state'
    }, 'required state'), p('LanguageSwitch needs to know about the languages in your app.'), Pre("\n// /assets/app.mjs\nexport const state = {\n  // ... other app state\n  languages: [\n    // the first language is the default fallback\n    { code: 'en', to: '/', text: 'english' },\n    { code: 'de', to: '/de/', text: 'deutsch' },\n  ],\n}\n"), p('thats it, your magic app now knows about your languages'), h2({
      id: 'source'
  ...
  '/language-switch/404/': function languageSwitch404() {
    return div('404 - not found');
  },
  '/language-switch/de/': function languageSwitchDe(state) {
    return [LanguageSwitch(state), h1('@magic-modules/language-switch'), p(['dies ist das ', Link({
      to: 'https://github.com/magic-modules'
    }, '@magic-modules'), ' LanguageSwitch modul. Es rendert ein Menu mit Sprachauswahl.']), GitBadges('magic-modules/language-switch'), h2({
      id: 'installation'
    }, 'installation:'), Pre('npm install @magic-modules/language-switch'), h2({
      id: 'usage'
    }, 'usage:'), p('in einer page oder einem komponent'), Pre("export const View = state => div({ class: 'page' }, LanguageSwitch(state))"), h2({
      id: 'source'
    }, 'source'), p(['der source dieser page ist im ', Link({
      to: 'https://github.com/magic-modules/language-switch/tree/master/example'
    }, 'example directory'), ' und wird mit ', Link({
      to: 'https://github.com/magic/core'
    }, '@magic/core'), ' gebaut und zu github publiziert.']), LightSwitch(state)];
  }
};
app({
